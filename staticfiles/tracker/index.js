document.addEventListener("DOMContentLoaded", () => {
    ...
    wGoal = document.querySelector('.hasProgress');
    currDiff = Number(document.querySelector('#currDiff').textContent);
    totalDiff = Number(document.querySelector('#totalDiff').textContent);
    prog = wGoal.querySelector('.progress-bar');
    bar = wGoal.querySelector('.progress');
    frac = (currDiff/totalDiff) * 100;
    if (frac > 100) {
        bar.style.width = `${frac}%`
        bar.style.background = 'green';
    }
    ...
    wGoal.querySelector('.perc').textContent = `${Math.round(frac)}%`;
